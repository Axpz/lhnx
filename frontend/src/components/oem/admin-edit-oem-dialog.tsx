...
import { useUpdateOEM, useProcessOEM } from '@/hooks/useOEM'
import { CreateOEMRequest, OEMSubmission, OEM_USER_TYPES, OEM_STATUS } from '@/lib/types'
import { cn } from '@/lib/utils'

interface AdminEditOEMDialogProps {
...
export function AdminEditOEMDialog({ request, open, onOpenChange, onSuccess }: AdminEditOEMDialogProps) {
    ...
    const { mutate: processOEM, isPending: isProcessing } = useProcessOEM()
            ...
            onSuccess: () => {
                ...
                if (statusChanged) {
                    processOEM({
                        ...
                        onError: (error: any) => {
                            console.error('OEM status update failed:', error)
                            if (error?.response?.data?.message) {
                                setSubmitError(error.response.data.message)
                            } else if (error?.message) {
                                setSubmitError(error.message)
                            } else {
                                setSubmitError('状态更新失败，请稍后重试')
                            }
                        }
                    })
                } else {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea
                            ...
                            placeholder="请描述特殊需求，如包装要求、材料要求、认证需求等"
                            rows={3}
                        />
                    </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4">
                        ...
                        <Button
                            ...
                            className="cursor-pointer"
                        >
                            {(isUpdating || isProcessing) ? '更新中...' : '更新需求'}
                        </Button>
                    </div>
                </form>
            </DialogContent>
